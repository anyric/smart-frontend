        ...
        >
            <template v-slot:top>
                <v-toolbar color="white">
                    <h2 class="mr-1" color="teal darken-1">Permissions</h2>
                    <v-spacer></v-spacer>
                    <v-text-field
                        v-model="search"
...
export default {
    ...
    methods: {
		...
		openDialog(item){
            ...
            this.dialogItems = this.permissions;
            this.itemType = "PERMISSIONS";
			this.isOpen = true;
		},
		...
		delete (data) {
			if (data.type == "PERMISSIONS") {
				this.$store.dispatch('DELETE_PERMISSION', data);
			}
		},

        checkPermissions() {
            const rec = this.collections.filter(item => item.name.toLowerCase() === "permissions");
